...
router.post("/cargarProceso", async function (req, res) {
  ...
  try {
    const massiveProcessingId = await asignarProceso(numero_de_contrato,nombre_persona_natural,ciudad_inmobiliaria,cedula,tarifa_segun_zona,fecha,nombre_representante_legal,cedula_representante_legal,nombre_establecimiento_comercio,numero_celular,correo);
